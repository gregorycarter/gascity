...
func TestEffectiveWorkQueryDefault(t *testing.T) {
	...
	if strings.Contains(got, `--sort oldest`) {
		t.Errorf("EffectiveWorkQuery() must preserve the ready reader's priority order: %q", got)
	}
...
`)
	if !strings.Contains(out, "ready-no-history") {
		t.Fatalf("EffectiveWorkQuery() did not pick routed work in reader order: %q", out)
	}
}
...
func TestEffectiveWorkQueryRoutedQueuePreservesReaderPriorityOrder(t *testing.T) {
	a := Agent{Name: "worker", Dir: "hello-world"}
	if got := a.EffectiveWorkQuery(); strings.Contains(got, "--sort oldest") {
		t.Fatalf("EffectiveWorkQuery() must not override the ready reader's priority order: %q", got)
	}
	out := runEffectiveWorkQuery(t, a, map[string]string{
		"GC_SESSION_ORIGIN": "ephemeral",
	}, `#!/bin/sh
...
case "$*" in
  *"--sort oldest"*)
    echo "routed probe must not override the reader's priority order" >&2
    exit 1
    ;;
...
`)
	if !strings.Contains(out, "newer-p0") {
		t.Fatalf("EffectiveWorkQuery() did not retain the reader's priority-ranked head: %q", out)
	}
}
...
`)
	if !strings.Contains(out, "priority-fallback") {
		t.Fatalf("EffectiveWorkQuery() did not pick routed fallback work in reader order: %q", out)
	}
}
