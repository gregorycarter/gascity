...
func installFakeDelegatedSystemctlHangingVerbWithUnitState(t *testing.T, verb string, isActiveExit int) string {
	...
	// written executable. Warm the shim before timeout-sensitive assertions;
	// discard the warm-up record so the log contains only calls under test.
	if err := exec.Command(shim, "warmup").Run(); err != nil {
		t.Fatalf("warming fake systemctl: %v", err)
	}
