...
| `inversion_priority` | integer |  | `0` | InversionPriority is the highest (numerically largest) priority class protected against inversion. Defaults to 0: only P0 beads are force-assigned past pool ordering. Pointer so an unset section marshals away: `omitempty` does not suppress a zero-valued int, and 0 is also a meaningful explicit setting here. |
...
| `max_actions_per_pass` | integer |  | `5` | MaxActionsPerPass caps mutating actions in a single pass, bounding the blast radius of any misdetection. Defaults to 5. Pointer for the same marshal reason as InversionPriority. |
