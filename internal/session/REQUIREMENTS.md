...
| SESSION-LIFE-009 | Heartbeat busy-hold vs user hold | A future `held_until` with empty `sleep_intent` (a `gc runtime heartbeat` busy-hold) displays as `busy-hold`, never `user-hold`, and does not suppress waking a non-live session that has assigned work. A suspend hold (`sleep_intent="user-hold"`) displays `user-hold` and keeps suppressing wake. Timer deferral for a live held session is unchanged. (2026-08-13 refinery user-hold display lie; discriminator mirrors the #3994 crash-recovery arm.) | `internal/session/lifecycle_projection_test.go`; `cmd/gc/compute_awake_set_test.go` (`TestComputeAwakeSet_HeartbeatBusyHoldWakeSuppression`); `cmd/gc/session_reconciler_test.go` (`TestReconcileSessionBeads_AsleepHeartbeatBusyHoldWakesForWork`); `cmd/gc/session_model_phase2_pin_spec_test.go` (`TestPhase2SessionListReason_HeartbeatBusyHoldShowsBusyHold`) |
